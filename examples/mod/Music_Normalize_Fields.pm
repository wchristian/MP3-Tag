...
sub _read_composer_file ($$*$$) {
  ...
  local $/ = "\n";		# allow customization
  if (defined $fh) {
    $f |= "composer's file" . (eval {' for ' . $self->composer} || '');
  } else {
...
sub load_composer ($$) {
  ...
  (my $myself = __FILE__) =~ s,.*[\\/],,;	# 'Music_Normalize_Fields.pm'
  my $f = $INC{$myself};
...
$Music_Normalize_Fields::translate_signature = \&translate_signature;
...
sub normalize_signature ($$$$) {
  ...
  $key = uc $key;
  $key = $key{$key} || $key;
  &$Music_Normalize_Fields::translate_signature($self,$key,$mod,$alteration);
}
...
sub opus_parser ($) {
  my $tag = shift;
  my $c = eval {$tag->composer} || $tag->artist;
...
## perl -MMusic_Normalize_Fields -wlne   "BEGIN {$tag = Music_Normalize_Fields::prepare_tag_object_comp(shift @ARGV); print q(# format = mail-header)} print Music_Normalize_Fields::emit_as_mail_header($tag,$_, 0,$pre)" gershwin Music_Normalize_Fields-G_Gershwin.comp-line >Music_Normalize_Fields-G_Gershwin.comp-mail1
...
## perl -MMusic_Normalize_Fields -wnle "BEGIN {$tag = Music_Normalize_Fields::prepare_tag_object_comp(shift @ARGV); print qq(# format = mail-header\n)} next unless s/^\s*\+\+\s*//; print Music_Normalize_Fields::merge_info($tag,$_, q(opus))" brahms o-brahms-op-no1-xslt
...
# perl -Ii:/zax/bin -MMusic_Normalize_Fields -wle "BEGIN{binmode $_, ':encoding(cp866)' for \*STDIN, \*STDOUT, \*STDERR}print Music_Normalize_Fields->check_persons"
sub check_persons ($) {
  my $self = shift;
  my %seen;
...
EOS
    ...
    print "$match ", $tag->composer, " ", $tag->album, "\n";
  }
}
...
in the directory named as the module file (without F<.pm>).  Works are stored
in F<.comp> files named after the shortened name of the composer.
...
exports the mentioned subroutines to the main namespace.  E.g., the last
example becomes:
